...
pub struct ClassicCluster {
    ...
    simulators: HashMap<Uuid, Arc<ClassicHandle>>,
...
impl ClassicCluster {
    ...
    pub async fn new_with_configuration(
        ...
        let mut peers = Vec::<Sender<ClassicMessage>>::with_capacity(total_number);
        let mut receivers = Vec::<Receiver<ClassicMessage>>::with_capacity(total_number);
        ...
        let fabric = Arc::new(new_classic_fabric(Arc::clone(&observer)));
        ...
        for (i, (rx, config)) in receivers.into_iter().zip(configs.into_iter()).enumerate() {
            let simulator = Arc::new(ClassicHandle::from_fabric(
                node_uuids[i],
                Arc::clone(&fabric),
            ));
    ...
    pub fn get_simulator<N: NodeRef>(&self, node: N) -> Option<&Arc<ClassicHandle>> {
