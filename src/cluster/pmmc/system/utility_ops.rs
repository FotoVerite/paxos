...
impl PmmcCluster {
    ...
    pub async fn crash_node(&self, node: usize) -> Option<Uuid> {
        ...
        if !self.process_manager.crash_node(crashed_uuid).await {
            return None;
        }
    ...
    pub async fn isolate_node(&self, node: usize) -> Option<Uuid> {
        ...
        if !self.process_manager.isolate_node(isolated_uuid).await {
            return None;
        }
        Some(isolated_uuid)
    }
    ...
    pub async fn heal_node(&self, node: usize) -> Option<Uuid> {
        ...
        if !self.process_manager.heal_node(to_heal).await {
            return None;
        }
        Some(to_heal)
    }
