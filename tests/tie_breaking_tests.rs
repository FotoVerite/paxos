...
async fn tie_breaking_same_round_higher_node_id_wins() {
    ...
    let b_node1 = Ballot::new(5, test_helpers::test_uuid(1)); // ballot (5, 1)
    let b_node2 = Ballot::new(5, test_helpers::test_uuid(2)); // ballot (5, 2) - higher node_id
...
async fn tie_breaking_lower_node_id_rejected_same_round() {
    ...
    let b_node2 = Ballot::new(5, test_helpers::test_uuid(2));
    let b_node1 = Ballot::new(5, test_helpers::test_uuid(1)); // Lower node_id
...
async fn ballot_ordering_complete_comparisons() {
    ...
    let b2_1 = Ballot::new(2, test_helpers::test_uuid(1));
    let b2_2 = Ballot::new(2, test_helpers::test_uuid(2));
...
async fn proposer_ballot_ordering_from_proposer_perspective() {
    ...
    let msg = proposer.propose(DecreeId(0), cmd.clone()).await;

    if let Message::Prepare { ballot, .. } = msg {
        // Proposer 1 should create ballot (1, 1)
        assert_eq!(ballot.number, 1);
        assert_eq!(ballot.node_id, test_helpers::test_uuid(1));
    } else {
...
async fn acceptor_rejects_lower_node_id_when_already_promised_to_higher() {
    ...
    let b_higher_node = Ballot::new(5, test_helpers::test_uuid(2));
    let b_lower_node = Ballot::new(5, test_helpers::test_uuid(1));
...
async fn tie_breaking_affects_accept_phase() {
    ...
    let b_node1 = Ballot::new(5, test_helpers::test_uuid(1));
    let b_node2 = Ballot::new(5, test_helpers::test_uuid(2)); // Higher due to node_id
